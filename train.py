...
def train(model, optimizer, loss_fn, data_iterator, metrics, params, num_steps):
    ...
    for i in t:
        ...
        train_batch, indices_batch = next(data_iterator)

        # compute model output and loss
        output_batch = model(train_batch)
        loss = loss_fn(output_batch, indices_batch)
        ...
        if i % params.save_summary_steps == 0:
            ...
            indices_batch = indices_batch.data.cpu().numpy()

            # compute all metrics on this batch
            summary_batch = {metric: metrics[metric](output_batch, indices_batch)
...
def train_and_evaluate(model, train_data, val_data, optimizer, loss_fn, metrics, params, model_dir, restore_file=None):
    ...
    for epoch in range(params.num_epochs):
        ...
        val_metrics, _ = evaluate(model, loss_fn, val_data_iterator, metrics, params, num_steps, True)
...
if __name__ == '__main__':
    ...
    model = net.MerchantNet().cuda() if params.cuda else net.MerchantNet()
